...
export type EspObject = {
	...
	SetTarget: (self: EspObject, target: TargetType) -> (),
...
local function CreateTrackedEsp(config: ESPConfig, instance: Instance): TrackedObject?
	...
	local function RegisterCleanup(item: any)
		if typeof(item) == "RBXScriptConnection" or typeof(item) == "Instance" or typeof(item) == "thread" then
	...
	if config.Tasks then
		for _, taskFunc in ipairs(config.Tasks) do
			if type(taskFunc) == "function" then
				local thread = task.spawn(function()
					taskFunc(tracked)
				end)
				table.insert(tracked.Cleanups, thread)
			end
		end
	end
...
function Esp.MakeEsp(espName: string)
	return function(config: ESPConfig): EspObject
		...
		function self:SetTarget(targetInput: TargetType)
			task.spawn(function()
				local resolvedTarget = if type(targetInput) == "function" then targetInput() else targetInput

				if typeof(resolvedTarget) == "Instance" then
					local rootInstance = resolvedTarget
					local function tryAdd(inst: Instance)
					...
					local added_con = rootInstance.DescendantAdded:Connect(tryAdd)
					local removed_con = rootInstance.DescendantRemoving:Connect(function(desc)
